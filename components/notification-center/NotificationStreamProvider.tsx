...
export type AnnouncementStreamEvent = {
  id: string
  title: string
  _deleted?: boolean
  _updated?: boolean
...
export function NotificationStreamProvider({ children }: { children: ReactNode }) {
  ...
  // ── Notifications: polling (see lib/notification-center/poll.ts for why) ──
  useEffect(() => {
    ...
    return startNotificationPolling(emitCount, emitNew)
  }, [])

  // ── Announcements: unchanged SSE connection ─────────────────────────────
  useEffect(() => {
    ...
    const emitAnnouncement = (data: AnnouncementStreamEvent) => {
      for (const l of listenersRef.current) l.onAnnouncement?.(data)
    }
    ...
    const connect = () => {
      ...
      es.addEventListener('announcement', (e) => {
        const data = safeParse<AnnouncementStreamEvent>((e as MessageEvent).data)
        if (data?.id) emitAnnouncement(data)
      })
      es.onerror = () => {
        es?.close()
        if (cancelled) return
