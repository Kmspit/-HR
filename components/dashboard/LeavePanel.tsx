...
const LEAVE_TYPES = [
  { value: 'SICK',     label: '🤒 ลาป่วย',         max: 30 },
  { value: 'VACATION', label: '🏖️ ลาพักร้อน',       max: 6 },
  { value: 'PERSONAL', label: '🗓️ ลากิจ',           max: 3 },
  { value: 'UNPAID',   label: '💸 ลาไม่รับค่าจ้าง', max: 99 },
]
          ...
          {leaves.length === 0 ? (
            ...
            <div key={l.id} className="rounded-2xl border border-white/5 bg-slate-900 p-4">
              <div className="flex items-center justify-between mb-2">
                <p className="font-semibold text-white">{LEAVE_TYPES.find(t => t.value === l.type)?.label ?? l.type}</p>
