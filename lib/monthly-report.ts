...
export async function buildMonthlyReport(month: number, year: number) {
  ...
  const rows = await Promise.all(
    employees.map(async (emp) => {
      ...
      const earlyLeaveDays = attendances.filter((a) => a.status === 'EARLY_LEAVE' || (a.earlyLeaveMinutes ?? 0) > 0).length
  ...
  return { month, year, generatedAt: new Date().toISOString(), employees: rows }
}
