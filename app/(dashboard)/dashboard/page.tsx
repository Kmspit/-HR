...
function StatCard({
  ...
  return (
    <div
      className="relative overflow-hidden rounded-2xl p-3.5 transition-all duration-200 hover:-translate-y-0.5"
      style={{ background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.07)', boxShadow: '0 4px 20px rgba(0,0,0,0.2)' }}
    >
      ...
      <div className="relative flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[10px] text-slate-500 leading-none">{label}</p>
          <p className="mt-1 text-2xl font-extrabold text-white leading-none">{value}</p>
          <p className="mt-0.5 text-[9.5px] text-slate-600 truncate">{sub}</p>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="p-4 md:p-5 space-y-5">
        ...
        {/* ─── Stat Cards ─── */}
        <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {/* Attendance Chart */}
          <div className="lg:col-span-2 rounded-2xl p-4 md:p-5"
            style={{ background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="font-semibold text-white text-[15px]">ภาพรวมการเข้างาน</h2>
                <p className="text-[11px] text-slate-500 mt-0.5">7 วันย้อนหลัง</p>
              </div>
          ...
          <div className="rounded-2xl p-4 md:p-5"
            style={{ background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-semibold text-white text-[15px]">กิจกรรมล่าสุด</h2>
              <span className="badge-blue text-[10px]">วันนี้</span>
            </div>
            <div className="space-y-2.5">
              {recentAttendance.length === 0 && (
                <p className="text-center text-sm text-slate-600 py-6">ยังไม่มีกิจกรรมวันนี้</p>
              )}
              {recentAttendance.map((a) => (
                <div key={a.id} className="flex items-start gap-2.5">
                  <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg text-[11px] font-bold text-white mt-0.5"
              ...
              {recentLeaves.slice(0, 3 - Math.min(recentAttendance.length, 3)).map((l) => (
                <div key={l.id} className="flex items-start gap-2.5">
                  <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-blue-500/20 text-[11px] font-bold text-blue-400 mt-0.5">
                    {l.user.name[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-semibold text-slate-200 leading-tight truncate">{l.user.name}</p>
                    <p className="text-[10px] text-slate-500 mt-0.5">ขอ{leaveTypeLabel[l.type]}</p>
                  </div>
                  <span className="badge-yellow" style={{ fontSize: '9px' }}>รอ</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="rounded-2xl p-4 md:p-5"
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="font-semibold text-white text-[15px]">คำขอลา</h2>
                <p className="text-[11px] text-slate-500 mt-0.5">รายการล่าสุด</p>
              </div>
              <Link href="/approvals" className="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1">
                อนุมัติ
                <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/></svg>
              </Link>
            </div>
            <div className="space-y-1.5">
              {recentLeaves.length === 0 ? (
                <p className="text-center text-sm text-slate-600 py-6">ไม่มีคำขอลา</p>
              ) : recentLeaves.map((l) => {
                const s = statusMap[l.status] ?? { label: l.status, cls: 'badge-slate' }
                return (
                  <div key={l.id} className="flex items-center gap-3 rounded-xl px-3 py-2.5 border border-white/[0.04] hover:bg-white/[0.03] transition-colors">
                    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl text-[11px] font-bold text-white"
                      style={{ background: 'linear-gradient(135deg,#3b82f6,#6366f1)' }}>
                      {l.user.name[0]}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="truncate text-[13px] font-semibold text-slate-200">{l.user.name}</p>
                      <p className="text-[10px] text-slate-500">{leaveTypeLabel[l.type]} · {l.user.department ?? '—'}</p>
                    </div>
          ...
          <div className="rounded-2xl p-4 md:p-5"
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="font-semibold text-white text-[15px]">มาสายวันนี้</h2>
                <p className="text-[11px] text-slate-500 mt-0.5">{lateToday.length} คน</p>
              </div>
              <Link href="/attendance" className="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1">
                ดูทั้งหมด
                <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/></svg>
            ...
            <div className="space-y-1.5">
              {lateToday.length === 0 ? (
                <div className="flex flex-col items-center py-6 gap-2">
                  <svg className="h-8 w-8 text-green-500/40" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                  <p className="text-sm text-slate-600">ไม่มีพนักงานมาสาย 🎉</p>
                </div>
              ) : lateToday.map((a) => (
                <div key={a.id} className="flex items-center gap-3 rounded-xl px-3 py-2.5 border border-yellow-500/10 bg-yellow-500/[0.04]">
                  <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl text-[11px] font-bold text-white"
                    style={{ background: 'linear-gradient(135deg,#f59e0b,#d97706)' }}>
                    {a.user.name[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="truncate text-[13px] font-semibold text-slate-200">{a.user.name}</p>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 rounded-2xl p-4 md:p-5"
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="font-semibold text-white text-[15px]">เงินเดือนรายแผนก</h2>
                <p className="text-[11px] text-slate-500 mt-0.5">เดือนปัจจุบัน</p>
              </div>
              <Link href="/payroll" className="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1">
                จัดการ
                <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/></svg>
              </Link>
            </div>
            {deptPayroll.length === 0 ? (
              <p className="text-center text-sm text-slate-600 py-6">ยังไม่มีข้อมูล Payroll เดือนนี้</p>
            ) : (
              <div className="space-y-3">
                  ...
                  return (
                    <div key={dept}>
                      <div className="mb-1.5 flex items-center justify-between text-[12px]">
                        ...
                        <span className="text-slate-400">{amount.toLocaleString('th-TH')} ฿</span>
                      </div>
                ...
                <div className="mt-3 flex items-center justify-between border-t border-white/[0.06] pt-3">
                  <span className="text-[12px] font-semibold text-slate-300">รวมทั้งหมด</span>
                  <span className="text-[13px] font-bold text-white">{totalPayroll.toLocaleString('th-TH')} ฿</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="rounded-2xl p-4 md:p-5"
            style={{ background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.07)' }}>
            <h2 className="mb-4 font-semibold text-white text-[15px]">Quick Actions</h2>
            <div className="grid grid-cols-2 gap-2">
              {[
                { href: '/employees',     label: 'พนักงาน',    icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z',  gradient: 'from-blue-500 to-indigo-500',   roles: ['MANAGER_HR'] as Role[] },
                { href: '/payroll',       label: 'เงินเดือน',  icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z', gradient: 'from-emerald-500 to-teal-500',   roles: ['MANAGER_HR'] as Role[] },
                { href: '/approvals',     label: 'อนุมัติ',     icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z',                                                                                                                                                                                                                  gradient: 'from-violet-500 to-purple-500', roles: ['MANAGER_HR','ADMIN'] as Role[] },
              ...
              ].filter(a => !a.roles || a.roles.includes(role)).map((a) => (
                <Link key={a.href} href={a.href}
                  ...
                  <span className="text-[11px] font-medium text-slate-400 group-hover:text-white transition-colors">{a.label}</span>
                </Link>
              ))}
            </div>
          </div>
