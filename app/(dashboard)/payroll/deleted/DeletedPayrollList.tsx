...
export type DeletedPayrollItem = {
  id: string
  employeeName: string
...
export default function DeletedPayrollList({ initialItems }: { initialItems: DeletedPayrollItem[] }) {
  const router = useRouter()
  const [items, setItems] = useState<DeletedPayrollItem[]>(initialItems)
  const [restoringId, setRestoringId] = useState<string | null>(null)

  const restore = async (id: string) => {
    if (!window.confirm('กู้คืน payroll รายการนี้กลับมาแสดงในระบบ?')) return
    setRestoringId(id)
    try {
      const { ok, data, status } = await apiJson(`/api/payroll/${id}/restore`, { method: 'POST' })
      if (!ok) { toast.error(apiErrorMessage(data, 'กู้คืนไม่สำเร็จ', status)); return }
      setItems((prev) => prev.filter((it) => it.id !== id))
      toast.success('กู้คืน payroll แล้ว')
      router.refresh()
    } catch {
  ...
  return (
    <div className="p-4 md:p-6 space-y-3 max-w-4xl">
      {items.length === 0 && (
        <p className="text-sm text-slate-400 dark:text-white/40">ไม่มี payroll ที่ถูกลบ</p>
      )}
      {items.map((item) => (
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 space-y-1">
              <div className="flex flex-wrap items-center gap-2 text-sm">
                {item.employeeId && <span className="font-mono text-slate-500 dark:text-white/40">{item.employeeId}</span>}
                <span className="font-medium text-slate-900 dark:text-white">{item.employeeName}</span>
                <span className="text-slate-500 dark:text-white/50">{item.monthLabel}</span>
              </div>
              <p className="text-sm text-slate-700 dark:text-white/70">
                ยอดสุทธิ ฿{fmt(item.netSalary)}
              </p>
