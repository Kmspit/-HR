...
type TabKey = 'general' | 'personal' | 'contacts' | 'assignments'

type Props = {
  /** = session.user.id — this page only ever shows/edits the viewer's own
   *  record, so every reused HR-facing component below (EmployeeProfileTab
   *  etc.) is simply pointed at the viewer's own id. Every one of those
   *  components' APIs already allows `id === session.user.id` regardless of
   *  role (see each route's own `if (id !== session.user.id)` guard) —
   *  built that way since Phase 1 step 8a/8b/8c, just never used for self
   *  until now. */
  userId: string
...
export default function ProfileClient({ userId, branchId, initial, recordInfo, editHistory }: Props) {
  ...
  const tabs: { key: TabKey; label: string }[] = [
    ...
    { key: 'personal', label: 'ข้อมูลส่วนตัว & ที่อยู่' },
    { key: 'contacts', label: 'ผู้ติดต่อ & บัญชีธนาคาร' },
    { key: 'assignments', label: 'ประวัติตำแหน่ง' },
  ]
  ...
  return (
    <div className="w-full p-4 md:p-6 lg:px-8 pb-28 md:pb-8 space-y-5 md:space-y-6">
      ...
      {/* Tab navigation */}
      <div className="flex gap-1 rounded-2xl dark:bg-white/[0.04] light:bg-slate-100 p-1 border dark:border-white/[0.07] light:border-slate-200 overflow-x-auto">
        {tabs.map((t) => (
          <button
            key={t.key}
            type="button"
            onClick={() => setActiveTab(t.key)}
            className={`flex-shrink-0 sm:flex-1 px-4 py-2.5 rounded-xl text-[13px] font-semibold whitespace-nowrap transition-all ${
              activeTab === t.key
                ? 'bg-green-600 text-white shadow-lg shadow-green-600/20'
                : 'dark:text-white/50 light:text-slate-500 hover:dark:text-white/80 hover:light:text-slate-800 hover:dark:bg-white/5 hover:light:bg-white'
            }`}
          >
            {t.label}
          </button>
        ...
        <div className="space-y-5 md:space-y-6">
          ...
          <section className="glass-card rounded-2xl p-5 md:p-6 space-y-4">
            ...
            <p className="text-[11px] dark:text-slate-500 light:text-slate-500">
              ที่อยู่ปัจจุบันและที่อยู่ตามทะเบียนบ้าน ย้ายไปแก้ที่แท็บ &ldquo;ข้อมูลส่วนตัว & ที่อยู่&rdquo; แล้ว
            </p>
          </section>
      ...
      <EmployeeProfileTab employeeId={userId} active={activeTab === 'personal'} />

      <EmployeeContactsBankTab employeeId={userId} active={activeTab === 'contacts'} canViewSensitive />

      <EmploymentAssignmentHistoryTab
        employeeId={userId}
        employeeName={displayName || initial.email}
      ...
      {/* Mobile sticky save — only meaningful on the "ทั่วไป" tab, the other
          tabs each save their own section inline via their own buttons */}
      {activeTab === 'general' && (
        <div
